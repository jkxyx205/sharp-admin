        ...
        if (StringUtils.isNotBlank(keywords)) {
            ...
            for (int i = 0; i < keywordArray.length; i++) {
                ...
                condition += " AND (mm_material.code like :"+paramName+" or mm_material.name like :"+paramName+" or specification like :"+paramName+" " +
                        "OR (batch_id in (select mm_profile.batch_id from mm_profile, mm_characteristic_value where\n" +
                        " mm_characteristic_value.reference_id = mm_profile.id AND mm_characteristic_value.value like :"+paramName+")))";
