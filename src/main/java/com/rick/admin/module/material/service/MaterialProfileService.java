...
public class MaterialProfileService {
    ...
    public Map<String, String> getCharacteristicText(Collection<String> materialIdBatchIdStringCollection) {
        Assert.notEmpty(materialIdBatchIdStringCollection, "不能为空");
        String sql = "select concat(material_id, ifnull(batch_id, '')) materialIdBatchIdString, group_concat(mm_characteristic_value.value order by mm_characteristic_value.id asc SEPARATOR ' ') characteristicText from mm_profile left join mm_characteristic_value on mm_profile.id = mm_characteristic_value.reference_id where concat(material_id, ifnull(batch_id, '')) IN (:materialIdBatchIdString)\n" +
